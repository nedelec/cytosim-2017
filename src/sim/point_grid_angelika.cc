...
{
#if ( 0 ) // enable for ANGELIKA
        // prevent steric interaction between a fiber and the solid from which it originated
        if (bb.fl.fiber()->buddy(0) == aa.pe.mecable()->buddy(0))
            return;
#endif
    const real len = aa.radius + bb.radius;
...
#if ( 1 ) // disable Fiber-Fiber steric for ANGELIKA
